...
public class Scope
{
    ...
    public static readonly IReadOnlyCollection<Scope> SystemScopes = new Scope[] {
        All,
        General,
        Mail,
        Online,
    ...
    public static bool IsSystemScope(Scope scope)
        => IsSystemScope(scope.Name);

    public static bool IsSystemScope(string name)
        => SystemScopes.Any(s => s.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
}

public static class ScopeExtensions
{
    public static bool IsSystemScope(this Scope scope)
        => Scope.IsSystemScope(scope);
}
