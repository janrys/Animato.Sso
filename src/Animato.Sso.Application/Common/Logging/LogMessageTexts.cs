namespace Animato.Sso.Application.Common.Logging;
public static class LogMessageTexts
{
    public const string ErrorLoading = "Error loading";
    public const string ErrorCreating = "Error creating";
    public const string ErrorUpdating = "Error updating";
    public const string ErrorDeleting = "Error updating";
    public const string ErrorLoadingScopes = $"{ErrorLoading} scopes";
    public const string ErrorCreatingScopes = $"{ErrorCreating} scopes";
    public const string ErrorUpdatingScopes = $"{ErrorUpdating} scopes";
    public const string ErrorDeletingScopes = $"{ErrorDeleting} scopes";
    public const string ErrorLoadingUsers = $"{ErrorLoading} users";
    public const string ErrorCreatingUsers = $"{ErrorCreating} users";
    public const string ErrorUpdatingUsers = $"{ErrorUpdating} users";
    public const string ErrorDeletingUsers = $"{ErrorDeleting} users";
    public const string ErrorLoadingRoles = $"{ErrorLoading} roles";
    public const string ErrorCreatingRoles = $"{ErrorCreating} roles";
    public const string ErrorUpdatingRoles = $"{ErrorUpdating} roles";
    public const string ErrorDeletingRoles = $"{ErrorDeleting} roles";
}
